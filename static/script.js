document.addEventListener('DOMContentLoaded', () => {
    ...
    async function sendData(formData) {
        ...
        try {
            ...
            if (formData.has('url')) {
                if (isSuspicious) {
                    resultDiv.innerHTML = `<p class="phishing">🔴 Phishing link </p>`;
                } else {
                    resultDiv.innerHTML = `<p class="safe">🟢 Safe link </p>`;
                }
            } else if (formData.has('text_input')) {
                if (isSuspicious) {
                    resultDiv.innerHTML = `<p class="phishing">🔴 Spam text </p>`;
                } else {
                    resultDiv.innerHTML = `<p class="safe">🟢 Not spam </p>`;
                }
            } else if (formData.has('file')) {
                if (isSuspicious) {
                    resultDiv.innerHTML = `<p class="phishing">🔴 Injected or suspicious links found</p>`;
                } else {
