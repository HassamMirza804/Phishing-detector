...
def analyze_urls_from_text(text):
    ...
    if not urls:
        return [{
            ...
            "heuristic_percentage": "0.00%",
            "safe_browse_percentage": "0.00%",
            "overall_suspiciousness": "0.00%"
        }]

    results = []
    MAX_HEURISTIC_SCORE = 8
    MAX_SAFE_BROWSE_SCORE = 100
    ...
    for url in set(urls):
        ...
        # Calculate percentage for heuristic check
        heuristic_percentage = (heuristic_score / MAX_HEURISTIC_SCORE) * 100
        
        # Safe browse score is already a percentage (100 or 0)
        safe_browse_percentage = safe_browse_score
        
        # Calculate overall suspiciousness by combining scores
        # We give the safe browse database a higher weight as it's a definite match
        overall_score_numerator = (heuristic_score * 0.5) + ((safe_browse_score / 100) * 8 * 0.5)
        overall_score_denominator = 8
        overall_suspiciousness = (overall_score_numerator / overall_score_denominator) * 100
        ...
        results.append({
            ...
            "heuristic_percentage": f"{heuristic_percentage:.2f}%",
            "safe_browse_percentage": f"{safe_browse_percentage:.2f}%",
            "overall_suspiciousness": f"{overall_suspiciousness:.2f}%"
        })
    return results
...
def predict():
    ...
    if 'file' in request.files:
        ...
        for url in set(all_urls):
            ...
            # This is where the overall suspiciousness is calculated
            overall_score_numerator = (heuristic_score * 0.5) + ((safe_browse_score / 100) * 8 * 0.5)
            overall_score_denominator = 8
            overall_suspiciousness = (overall_score_numerator / overall_score_denominator) * 100
