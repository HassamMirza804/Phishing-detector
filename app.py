...
def analyze_urls_from_text(text):
    ...
    if not urls:
        return [{"url": "No URLs found", "heuristic_check": {"result": "N/A", "reason": ""}, "safe_browse_check": "N/A", "suspiciousness_percentage": "0.00%"}]

    results = []
    for url in urls:
        is_phishing, heuristic_reason, score = is_heuristically_phishing(url)
        safe_browse_result = is_safe_browse_safe(url)
        
        # Calculate percentage based on max possible score (8)
        # and format it to two decimal places
        percentage = (score / 8.0) * 100
        ...
        results.append({
            ...
            "suspiciousness_percentage": f"{percentage:.2f}%"
        })
    return results
...
def predict():
    ...
    if 'file' in request.files:
        ...
        if not all_urls:
            return jsonify([{"url": "No URLs found", "heuristic_check": {"result": "N/A", "reason": ""}, "safe_browse_check": "N/A", "suspiciousness_percentage": "0.00%"}])
